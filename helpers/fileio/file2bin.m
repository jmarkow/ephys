...
if iscell(filename)
	...
	tmp=uiputfile('*.bin','New filename');
...
store_datavec=zeros(1,buffersize,'int16'); % signed int16, initialize to reasonable number, cut off unused samples
counter=1;
...
for ii=1:nfiles
	...
	for i=1:datasamples
		for j=1:length(chidx)
			store_datavec(counter)=int16(datastruct.ephys.data(i,chidx(j)));
			counter=counter+1;
		end
	end
...
fid=fopen([ basefilename '.bin'],'Wb');
fwrite(fid,store_datavec,'int16','ieee-le'); % little-endian
fclose(fid);

% write out the parameter file

fid=fopen([ basefilename '.ofi' ],'w');

fprintf(fid,'NChan=%i\n',nchannels);
fprintf(fid,'Offset=0\n');
fprintf(fid,'Freq=%g\n',fs);
fprintf(fid,'MaxMV=%g\n',5);
fprintf(fid,'ResolutionBits=16\n');
fprintf(fid,'Swap=0');
