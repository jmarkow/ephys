function [CONTOUR_SPECT]=ephys_visual_lfp_tf_contour(EPHYS,HISTOGRAM,varargin)
%generates a song-aligned average spectrogram of the LFP using the contour method
%
%	[CONTOUR_SPECT=ephys_lfp_tf_contour(EPHYS.data,HISTOGRAM,EPHYS.labels,varargin)
%
%	EPHYS.data
...
for i=1:length(channels)
	...
	if ~isempty(savedir)
		save(fullfile(savedir, [savefilename num2str(channels(i)) '.mat']),'EPHYS.labels','channels','conn_comp',...
			'wsigma','angles','t','f');
	end
...
CONTOUR_SPECT.channels=EPHYS.labels;

if ~isempty(savedir)
	save(fullfile(savedir,'lfp_tf_data.mat'),'EPHYS.labels','channels','CONTOUR_SPECT','angles','wsigma');
end
