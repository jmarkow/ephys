...
%and complex averaging of all channels
%
...
proc_data=ephys_condition_signal(proc_data,'l','freq_range',freq_range,...
	'medfilt_scale',medfilt_scale,'medfilt',0,...
...
parfor i=1:ntrials
	...
	for j=1:ntimescales
		...
		for k=1:nchannels
			...
			[stft dx]=chirp_stft(currdata(:,k),'fs',proc_fs,'n',lfp_n,...
				'overlap',lfp_overlap,'nfft',lfp_nfft,'wsigma',wsigma(j));
...
CONTOUR_SPECT.gabor=gabor_ave;
CONTOUR_SPECT.consensus=consensus_ave;
...
fig_title=['CHAVE' num2str(channels) ' NTRIALS' num2str(ntrials)];
...
[ savefilename num2str(channels) ],'png','res',100);
close([spect_fig]);
